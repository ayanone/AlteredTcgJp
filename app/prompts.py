...
カード番号が読み取れない場合は card_number=null, unique_number=null としてください。

【宝石マーク（カード名の上）によるレアリティ判定】
カード番号の記載がよく読み取れない場合はこちらと合わせて推定してください。
・宝石マークなし（白い円） → C / H / T
・青色の宝石マーク → R または F
・銅色の宝石マーク → E（エグザルテッド）
・金色の宝石マーク → U（ユニーク）
...
・能力欄の識別方法と読み取り方:
  カード下部のテキスト欄は2つの領域に分かれています。ECHO_EFFECTの領域を持っていないカードも存在します。能力欄に斜体で書かれているテキストは無視してください。
...
[
    {
        ...
        "rarity": "R など1文字（読み取れない場合は宝石マークと合わせて推定）",
